...
.db.obj.info <- function (db.obj_name, con.id = 1)
{
    ...
    if (length(parts) == 2) {
        ...
        table_name <- parts[1]
        table_schema <- dbGetQuery(.localVars$db[[con.id]]$con,
...
.is.table <- function (db.obj_name, con.id = 1)
{
    table.info <- .db.obj.info(db.obj_name, con.id)
    pick <- dbGetQuery(.localVars$db[[con.id]]$con,
...
.is.view <- function (db.obj_name, con.id = 1)
{
    table.info <- .db.obj.info(.db.obj_name, con.id)
    pick <- dbGetQuery(.localVars$db[[con.id]]$con,
